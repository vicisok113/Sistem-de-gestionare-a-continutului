...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                <?php
                $query = $this->db->query("SELECT * FROM categorii_produse WHERE id = '". $name ."'");
                ...
                ?>
                    ...
                    <div class="panel-heading">
                        <h3 class="panel-title">Editare categorie:</h3>
                    </div>
                    <div class="panel-body">
                     <?php $attributes = array('class' => '', 'role' => 'form');
                     echo form_open('index.php/admin/edit_cat_produse_validation/'.$id, $attributes);?>
                            <fieldset>
                            <p><?php echo validation_errors(); ?></p>
                                <div class="form-group">
                                    <label>Nume categorie:</label>
                                    <input class="form-control" value="<?php echo $categorie1; ?>" placeholder="Nume categorie" name="categorie" type="text" autofocus>
                                </div>
                                <div class="form-group">
                                <label>Categorie parinte: </label>
                                    <?php echo ' <select class="form-control" name="parinte">';
                                    ?> <option value="0">Niciuna</option>
                                    <?php $query1 = $this->db->query("SELECT * FROM categorii_produse WHERE parinte = '0'");
                                    foreach ($query1->result() as $row)
                                    {
                                        $id1 = $row->id;
                                        $categorie = $row->categorie;
                                        if($parinte1 == $id1) {
                                        ?> <option value="<?php echo $id1; ?>" selected><?php echo $categorie; ?></option>';
                                        <?php } else { ?>
                                            <option value="<?php echo $id1; ?>"><?php echo $categorie; ?></option>';
                                        <?php }
                                    }
                                    echo '</select>'; ?>
                                </div>
